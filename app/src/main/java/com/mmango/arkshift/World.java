...
public class World {
	...
	public static final int NO_OBJECT_ID = -1; // an empty cell
	...
	boolean prepared;
	float timePassed = 0f;
	int[] cellIds = new int[12]; // for calculation the ball's rectBounds
									// vertices
	int cellsPerRow;
	int cellsPerCol;
	...
	private void setNoCellIds() {
		for (int z = 1; z < 12; z++) {
			cellIds[z] = NO_OBJECT_ID;
		}
	}

	private boolean getCellIds(Ball ball, boolean aboveRacquet) {
		// order of cells:
		// 1) x1y1 - left bottom corner
		// 2) x2y2 - right top corner
		// 3) x3y3 - right bottom corner
		// 4) x4y4 - left top corner

		//populate cellIds[] with the NO_OBJECT_ID 
		...
		if (aboveRacquet) {
			...
			if (y2 > level - 1) { // if the right up corner of the ball rect is
				...
				// "right up corner of the ball rect is in cell y2 = " + y2
			...
			// Log.d("World:getCellIds", "The ball is in cells: x1=" + x1 +
			// " y1="
			// + y1 + " x2=" + x2 + " y2=" + y2);
...
//				Log.d("World:getCellIds", "The ball is completely in one cell");
				// the ball is completely in one cell so no need to duplicate
				// the cell IDs. One is enough
				if (y1 > 0) {
					cellIds[8] = x1;
					cellIds[9] = y1 - 1;
				}
...
//				Log.d("World:getCellIds",
//						"The ball overlaps only two cells either in same column or same row");
...
//				Log.d("World:getCellIds", "The ball overlaps four cells");
		...
		} else { // the ball is under racquet's central horisontal line, e.g.
					// aboveRacquet = false

			int y1 = (int) Math
					.floor((ball.rectBounds.lowerLeft.y - FRAME_WIDTH)
							/ CELL_SIZE);

			if (y1 > level - 1) { // if the left bottom corner of the ball rect
									// is higher than the highest possible row
									// then no collision is possible
//				Log.d("World:getCellIds",
...
//								+ " which is higher than the highest possible row");
			...
			int y2 = (int) Math.floor((ball.rectBounds.lowerLeft.y
					- FRAME_WIDTH + ball.rectBounds.height)
					/ CELL_SIZE);
			...
			if (x1 == x2 && y1 == y2) {
//				Log.d("World:getCellIds", "The ball is completely in one cell");
				// the ball is completely in one cell
				// so no need to duplicate the cell IDs. One is
				// enough
				if (y1 > 0) {
					cellIds[8] = x1;
					cellIds[9] = y1 - 1;
...
//		for (int i = 0; i < 12; i++) {
//			Log.d("World:getCellIds", "cellIds[" + i + "]=" + cellIds[i]);
//		}
	...
	private void checkBallCollisionsWithBricks() {
		...
		// 3) x3y3 - right bottom corner
		// 4) x4y4 - left top corner
		// 5) x5y5 - just above the left top corner (above x4y4).
		// 6) x6y6 - just above the right top corner (above x2y2).
		// I need to check x5y5 and x6y6 in order to catch the moving bricks
		int[] bricksToCheck = { NO_OBJECT_ID, NO_OBJECT_ID, NO_OBJECT_ID,
				NO_OBJECT_ID, NO_OBJECT_ID, NO_OBJECT_ID }; // 4 bricks ids for
		// the maximum
		// possible cells +
		// 2 ids for the
		// cells to which
		...
		if (ball.position.y > racquet.position.y) {
//			Log.d("World:checkBallCollisionsWithBricks",
//					"Getting the potential cells from the ceiling");
			// get the potential cells from the ceiling
			if (getCellIds(ball, true)) { 
				// do these operations only if the ball is not completely out of the grid, either ceiling or floor.
				for (int z = 0; z < 12; z++) {
					int w = z + 1;
					if ((cellIds[z] == NO_OBJECT_ID)
							|| (cellIds[w] > level - 1)) {
						z = w;
						continue;
					}
//					Log.d("World:checkBallCollisionsWithBricks", "z=" + z
//							+ " w=" + w);
...
//							+ cellIds[z] + " row=" + cellIds[w]);
		...
		} else { // lower than the racquet:
//			Log.d("World:checkBallCollisionsWithBricks",
//					"Getting the potential cells from the floor");
			// get the potential cells from the floor
			if (getCellIds(ball, false)) { // do these operations only if the
											// ball is not
				// completely out of the grid, either
				// ceiling or floor.
				for (int z = 0; z < 12; z++) {
					int w = z + 1;
					if ((cellIds[z] == NO_OBJECT_ID)
...
//		Log.d("World:checkBallCollisionsWithBricks",
//				"Printing out the content of bricksToCheck:");
//		for (int i = 0; i < 6; i++) {
//			Log.d("World:checkBallCollisionsWithBricks",
//					"------ bricksToCheck[" + i + "]=" + bricksToCheck[i]);
//		}
		...
		for (int i = 0; i < 6; i++) {
			...
			if (bricksToCheck[i] != NO_OBJECT_ID) {
//				Log.d("World:checkBallCollisionsWithBricks",
//						"AGAIN bricksToCheck[" + i + "]=" + bricksToCheck[i]);
...
//					Log.d("World:checkBallCollisionsWithBricks",
//							"Overlapping brick " + bricksToCheck[i]);
...
//			Log.d("World:checkBallCollisionsWithBricks", "bricksAffected[" + i
//					+ "] = " + bricksAffected[i]);

			listener.hitAtBrick();
			if (brick.state != Brick.BRICK_STATE_STILL) {
//				Log.d("World:checkBallCollisionsWithBricks",
//						"It is a Moving brick, we don't count it as affected. ID = "
//								+ bricksAffected[i]);
				continue;
			}
			...
			if (brick.atCeiling) {
				...
				// Log.d("World:checkBallCollisionsWithBricks",
				// "A collision with a bottom brick just happened!");
				// the bricks on the ceiling will shift down
				for (int y = level - 1; y > 0; y--) {
					ceilingBricksId[column][y] = ceilingBricksId[column][y - 1];
					if (ceilingBricksId[column][y] != NO_OBJECT_ID) {
						bricks.get(ceilingBricksId[column][y]).setCell(column,
